...
>
    @if (Auth::id() === $comment->user_id || Auth::id() === $comment->post->author_id)
        <button
            ...
            @click="confirm('Are you sure you want to delete this comment?') && window.livewire.emit('deleteComment', {{ $comment->id }})"
            aria-label="Close"
        >
    ...
    <p class="comment__byline">
        {{ $comment->user->name }} commented <time pubdate datetime="{{ $comment->created_at }}">{{ $comment->created_at->diffForHumans() }}:
    </p>
